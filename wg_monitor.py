...
HISTORY_POINTS  = 60     # 60 secondes d'historique
...
class AnimatedBar(ctk.CTkFrame):
    ...
    def set_value(self, pct: float):
        ...
        if pct >= 85:   col = T["red"]
        elif pct >= 65: col = T["orange"]
        else:           col = self._color
...
# ─────────────────────────────────────────────
#  HISTORY GRAPH (axe fixe 0–100%)
# ─────────────────────────────────────────────
class HistoryGraph(ctk.CTkCanvas):
    """Graphe avec axe Y fixe 0-100, grille de fond, label en overlay."""
    ...
    def _draw(self):
        ...
        # Valeur actuelle en overlay
        cur = self._data[-1] if self._data else 0
        if cur >= 85:   col = T["red"]
        elif cur >= 65: col = T["orange"]
        else:           col = self._color
...
# ─────────────────────────────────────────────
#  MINI GRAPH (réseau — axe dynamique)
# ─────────────────────────────────────────────
...
class WGMonitor(ctk.CTk):
    def __init__(self):
        ...
        self._proc_sort     = "cpu"   # "cpu" ou "ram"
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(
            ...
            text_color=T["text_sec"], corner_radius=8, height=32, width=140,
            command=self._open_theme_popup,
        ).pack(side="right", padx=(6, 16), pady=10)

        # Bouton copier config
        self._copy_btn = ctk.CTkButton(
            topbar, text="📋  Copier config",
            font=("Consolas", 11),
            fg_color=T["bg_card2"], hover_color=T["bg_border"],
            text_color=T["text_sec"], corner_radius=8, height=32, width=150,
        ...
        for tab_id, label, icon in [
            ...
            btn = ctk.CTkButton(
                ...
                hover_color=T["bg_card2"],
                text_color=T["accent"] if active else T["text_sec"],
                corner_radius=6, height=28, width=150,
                border_width=0,
        ...
        ctk.CTkLabel(bot, text="WG Monitor v1.0  •  Python + customtkinter",
                      font=("Consolas", 11), text_color=T["text_mut"]).pack(side="right", padx=16)
    ...
    def _build_dashboard(self, parent):
        ...
        top_cpu = ctk.CTkFrame(cpu_body, fg_color="transparent")
        top_cpu.pack(fill="x")
        top_cpu.columnconfigure(0, weight=1)
        top_cpu.columnconfigure(1, weight=2)

        # Gauche : big % + infos
        left_cpu = ctk.CTkFrame(top_cpu, fg_color="transparent")
        left_cpu.grid(row=0, column=0, sticky="nsew", padx=(0, 10))

        self._cpu_big = ctk.CTkLabel(left_cpu, text="0%",
                                      font=("Consolas", 38, "bold"),
                                      text_color=T["accent"])
        self._cpu_big.pack(anchor="w")
        ctk.CTkLabel(left_cpu, text=self._short_cpu(),
                     font=("Consolas", 11), text_color=T["text_mut"],
                     wraplength=220, justify="left").pack(anchor="w", pady=(0, 6))

        self._core_bars = []
        cgrid = ctk.CTkFrame(left_cpu, fg_color="transparent")
        cgrid.pack(fill="x")
        ...
        self._cpu_freq = ctk.CTkLabel(left_cpu, text="",
                                       font=("Consolas", 11),
                                       text_color=T["text_mut"])
        self._cpu_freq.pack(anchor="w", pady=(6, 0))

        # Droite : graphe historique CPU
        right_cpu = ctk.CTkFrame(top_cpu, fg_color=T["bg_card2"], corner_radius=8)
        right_cpu.grid(row=0, column=1, sticky="nsew")
        ctk.CTkLabel(right_cpu, text="Historique CPU  (60s)",
                     font=("Consolas", 11), text_color=T["text_mut"]).pack(anchor="w", padx=10, pady=(8, 4))
        self._cpu_graph = HistoryGraph(right_cpu, color=T["accent"],
        ...
        top_ram = ctk.CTkFrame(ram_body, fg_color="transparent")
        top_ram.pack(fill="x")
        top_ram.columnconfigure(0, weight=1)
        top_ram.columnconfigure(1, weight=2)

        left_ram = ctk.CTkFrame(top_ram, fg_color="transparent")
        left_ram.grid(row=0, column=0, sticky="nsew", padx=(0, 10))

        self._ram_big = ctk.CTkLabel(left_ram, text="0%",
                                      font=("Consolas", 38, "bold"),
                                      text_color=T["blue"])
        self._ram_big.pack(anchor="w")
        self._ram_detail = ctk.CTkLabel(left_ram, text="",
                                         font=("Consolas", 11),
                                         text_color=T["text_mut"])
        self._ram_detail.pack(anchor="w", pady=(0, 8))
        self._ram_bar  = AnimatedBar(left_ram, label="RAM",  color=T["blue"])
        self._ram_bar.pack(fill="x", pady=2)
        self._swap_bar = AnimatedBar(left_ram, label="SWAP", color=T["accent2"])
        self._swap_bar.pack(fill="x", pady=2)

        mem_grid = ctk.CTkFrame(left_ram, fg_color=T["bg_card2"], corner_radius=8)
        ...
        # Droite : graphe historique RAM
        right_ram = ctk.CTkFrame(top_ram, fg_color=T["bg_card2"], corner_radius=8)
        right_ram.grid(row=0, column=1, sticky="nsew")
        ctk.CTkLabel(right_ram, text="Historique RAM  (60s)",
                     font=("Consolas", 11), text_color=T["text_mut"]).pack(anchor="w", padx=10, pady=(8, 4))
        self._ram_graph = HistoryGraph(right_ram, color=T["blue"],
        ...
        for p in psutil.disk_partitions()[:4]:
            try:
                ...
                if letter == "C":   label = "C:  —  Windows"
                elif letter == "D": label = "D:  —  Linux"
                else:               label = f"{letter}:  —  Disque externe"
    ...
    def _build_processes(self, parent):
        ...
        # Header avec tri
        hdr = ctk.CTkFrame(body, fg_color=T["bg_card"],
        ...
        sort_frame.pack(side="right", padx=16, pady=8)

        ctk.CTkLabel(sort_frame, text="Trier par :",
                     font=("Consolas", 11), text_color=T["text_mut"]).pack(side="left", padx=(0, 8))

        self._sort_cpu_btn = ctk.CTkButton(
            sort_frame, text="⚙️ CPU",
        ...
        # Table header
        cols = [("PID", 80), ("Processus", 300), ("CPU %", 100), ("RAM", 110), ("Statut", 100)]
        ...
        for cname, cw in cols:
            ctk.CTkLabel(col_frame, text=cname,
                         font=("Consolas", 11, "bold"),
                         text_color=T["accent"],
                         width=cw, anchor="w").pack(side="left", padx=10, pady=6)

        # Lignes de processus (10 lignes fixes)
        self._proc_rows = []
    ...
    def _set_proc_sort(self, mode: str):
    ...
    def _update_processes(self):
        try:
            ...
            key = "cpu" if self._proc_sort == "cpu" else "ram"
            ...
            for i, cells in enumerate(self._proc_rows):
                if i < len(top):
                    ...
                    cells["PID"].configure(text=str(p["pid"]),
                                            text_color=T["text_mut"])
                    cells["Processus"].configure(text=p["name"][:38],
                                                  text_color=T["text_pri"])
                    cells["CPU %"].configure(text=f"{p['cpu']:.1f}%",
                                              text_color=cpu_col)
                    cells["RAM"].configure(text=self._fmt(p["ram"]),
                                            text_color=T["blue"])
                    status_col = (T["green"] if p["status"] == "running"
                                  else T["text_mut"])
                    cells["Statut"].configure(text=p["status"],
        ...
        try:
            ...
            for p in psutil.disk_partitions()[:4]:
                try:
                    ...
                    letter = (letter[-1] if letter else "?") + ":"
                    disk_lines.append(
                        f"  {letter}  {self._fmt(u.used)} / {self._fmt(u.total)} ({u.percent:.1f}%)")
                except Exception:
                    pass
